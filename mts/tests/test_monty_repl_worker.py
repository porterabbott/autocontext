...
from mts.harness.repl.types import ReplCommand, ReplWorkerProtocol
...
class TestMontyReplWorkerExecution:
    def test_simple_expression_captured_via_print(self) -> None:
        ...
        w = _worker()
        with patch("mts.harness.repl.monty_worker._create_repl_monty", return_value=mock):
    ...
    def test_runtime_error_captured(self) -> None:
        ...
        w = _worker()
        with patch("mts.harness.repl.monty_worker._create_repl_monty", return_value=monty):
...
class TestMontyReplWorkerAnswer:
    ...
    def test_answer_persists_across_turns(self) -> None:
        ...
        w = _worker()
        with patch("mts.harness.repl.monty_worker._create_repl_monty", return_value=mock1):
            w.run_code(ReplCommand('answer["content"] = "step1"'))
        assert w.namespace["answer"]["content"] == "step1"

        with patch("mts.harness.repl.monty_worker._create_repl_monty", return_value=mock2):
...
class TestTrailingExpressionConversion:
    def test_trailing_expr_converted_to_print(self) -> None:
        from mts.harness.repl.monty_worker import _rewrite_trailing_expr
...
class TestMontyReplImportGuard:
    def test_import_error_when_monty_missing(self) -> None:
        ...
        w = _worker()
        with patch("mts.harness.repl.monty_worker._create_repl_monty", side_effect=ImportError("no pydantic_monty")):
            result = w.run_code(ReplCommand("x = 1"))
        assert result.error is not None
