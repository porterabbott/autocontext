...
class TestHTTPNotifier:
    def test_sends_json(self):
        with patch("mts.notifications.http.urllib.request.urlopen") as mock_urlopen:
    ...
    def test_swallows_errors(self):
        with patch("mts.notifications.http.urllib.request.urlopen", side_effect=Exception("fail")):
            n = HTTPNotifier("https://example.com/hook")
            e = NotificationEvent(type=EventType.FAILURE, task_name="test")
            n.notify(e)  # Should not raise
...
class TestSlackWebhookNotifier:
    def test_sends_blocks(self):
        with patch("mts.notifications.slack.urllib.request.urlopen") as mock_urlopen:
    ...
    def test_includes_channel(self):
        with patch("mts.notifications.slack.urllib.request.urlopen") as mock_urlopen:
            n = SlackWebhookNotifier("https://hooks.slack.com/test", channel="#mts-alerts")
            e = NotificationEvent(type=EventType.COMPLETION, task_name="test")
            n.notify(e)

            body = json.loads(mock_urlopen.call_args[0][0].data)
            assert body["channel"] == "#mts-alerts"

    def test_swallows_errors(self):
        with patch("mts.notifications.slack.urllib.request.urlopen", side_effect=Exception("fail")):
            n = SlackWebhookNotifier("https://hooks.slack.com/test")
            e = NotificationEvent(type=EventType.FAILURE, task_name="test")
            n.notify(e)
