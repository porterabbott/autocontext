...
from mts.harness.repl.session import RlmSession
from mts.harness.repl.types import ReplWorkerProtocol
...
class TestMontyReplWorkerWithSession:
    ...
    def test_session_multi_turn_with_state(self) -> None:
        ...
        with patch("mts.harness.repl.monty_worker._create_repl_monty", side_effect=lambda **kw: next(mock_iter)):
            session = RlmSession(
                client=client,
                worker=worker,
                role="analyst",
